...
def stored_graphs(condition):
    ...
    for f in graphFiles:
        ...
        if len(lines)==0:
            continue
        edges = [edge[:-1].split(",") for edge in lines if not edge.startswith('#')]
...
class eGreedyBysP(BysP,RND):
    ...
    def calculateEpsilon(self):
        c=self._getC()
        d=self._getD()
        pulls=len(set([n for n in self._G.nodes() if self._G.node[n]["isLead"] is not None]))
        K=len(set([v for v in self._G.nodes() if self._G.node[v]["isLead"]==True ]))
        return (c*K)/(d*pulls)
    def _getC(self):
        return 1.0
    def _getD(self):
...
class UCB(BysP):
    ...
    def addall(self, discovered, v, v_is_lead):
        ...
        leads = set([x for x in self._G.nodes() if self._G.node[x]["isLead"]==True ])
        if v is not None:
            if isLogEnabled('UCB') :
    ...
    def calculateExplorationValues(self, G, leads, v, notleads):
        ...
        for v in leads:
            G.node[v]["exploration"] = self._getXplorationFactor(self.hits.get(v,0), total_pulls)
        ...
        if isLogEnabled('UCB'):
            for l in leads:
                ...
                for v in openList:
                    ...
                    ranks[v] = P
                    if isLogEnabled('UCB') :
                        max_rs=[r for r in ranks if ranks[r]==max(ranks.values())]
                        for r in max_rs:
...
class UCB_COM(UCB):
    ...
    def calculateExplorationValues(self, G, leads, v, notleads):
        ...
        nodes=G.nodes()
        total_pulls = len(set([v for v in nodes if G.node[v]["isLead"] is not None]))
        for v in self.exploration:
            self.exploration[v] = self._getXplorationFactor(self.exploration[v], total_pulls)
...
class UCB_COM_RESET(UCB):
    ...
    def addall(self, discovered, v, v_is_lead):
        ...
        if v is None: return
        ns = set(G.neighbors(v))
        ns &= set([v for v in self._G.nodes() if self._G.node[v]["isLead"]==True])
        if not len(ns): return
        ns=tuple(sorted(ns))
...
#                self.arm_hits[ns] = [self.total_pulls,1]
    ...
    def calculateExplorationValues(self, G, leads, v, notleads):
        ...
        for v in self.arm_hits:
            self.exploration[v] = self._getXplorationFactor(self.arm_hits[v][1], self.total_pulls-self.arm_hits[v][0])
...
def arms_stat_begin():
    ...
    armf = csv.DictWriter(file("arms_%s.csv","w"),fieldnames,"",lineterminator="\n")
...
class SN_UCB(UCB):
    ...
    def findOptimalArm(self,justExpanded):
        ...
        if len(self.arm_hits)==0:
            ...
            maxArm = max(self.arm_hits, key=lambda x: self.realPromise(x, justExpanded,targetID))
    ...
    def writeArmStats(self, selected,v):
        leads = set([_ for _ in self._G.nodes() if self._G.node[_]["isLead"] == True])
        noleads = set([_ for _ in self._G.nodes() if self._G.node[_]["isLead"] == False])
        potentials = set([_ for _ in self._G.nodes() if self._G.node[_]["isLead"] == None])
        optimalArm, optimalNeigbors, real_optimal = self.findOptimalArm(v)
        dead_arms = [_ for _ in self.arm_hits if len(neighborsOfArm(self._G,_) & potentials)==0]
        new_arms = [_ for _ in self.arm_hits if neighborsOfArm(self._G,_) <= potentials and _ not in dead_arms]
        ...
        if len(set(nsNeigbors) & (leads | noleads))==1:
            ...
            selected_bysp= 1.0 - reduce(operator.mul,[1.0 - self._getPromiseFactor(1.0*len(set(G.neighbors(l)) & leads),1.0*len(set(G.neighbors(l)) & noleads)) for l in selected],1.0)
            optimal_bysp = 1.0 - reduce(operator.mul,[1.0 - self._getPromiseFactor(1.0*len(set(G.neighbors(l)) & leads),1.0*len(set(G.neighbors(l)) & noleads)) for l in optimalArm],1.0)
    ...
    def addall(self, discovered, v, v_is_lead):
        if examine_arms and v is not None:
            leads = set([vl for vl in self._G.nodes() if self._G.node[vl]["isLead"]==True])
    ...
    def getRankOfArm(self, ns):
        ...
        if not len(ns): return
        leads = set([vl for vl in self._G.nodes() if self._G.node[vl]["isLead"]==True])
        notleads = set([vl for vl in self._G.nodes() if self._G.node[vl]["isLead"]==False])
        P = self.exploitation[ns]#*float(len(ns))/max([len(arm) for arm in self.arm_hits]+[1.0])
        if ns in self.arm_hits:
    ...
    def allArmCombinations(self,leads=None):
        if leads is None:
            leads = set([v for v in self._G.nodes() if self._G.node[v]["isLead"]==True])
    ...
    def calculateExploitationValues(self, G, leads, notleads, potentials):
        ...
        for v in openList:
            ...
            if L+NL ==0 and len(arm)>2 and len(leadsFound)>0:
                ...
                continue
            P = 1.0 - reduce(operator.mul,[1.0 - self._getPromiseFactor(1.0*len(set(G.neighbors(l)) & leads),1.0*len(set(G.neighbors(l)) & notleads)) for l in arm],1.0)
            if arm not in self.arm_hits:
                self.arm_hits[arm] = [totalPulls, L+NL+VIRTUAL]
            else:
                self.arm_hits[arm][1] = L+NL+VIRTUAL
                self.exploitation[arm] = (self._getPromiseFactor(L,NL) * (L+NL) + VIRTUAL * P) / (VIRTUAL+L+NL)
                #handle unknown arms
                for n_arm in unknown_arms:
                    ...
                    if lastLeadFound in possible_arm:
                        ...
                        if possible_arm in self.exploitation.keys():
                            friendsOf=reduce(lambda s1,s2:s1&s2,[set(G.neighbors(l)) for l in possible_arm])
                            howmuch=len(newPotentials & friendsOf)
                            possible_arm_score = self.exploitation[possible_arm]
                            P = 1.0 - reduce(operator.mul,[1.0 - self._getPromiseFactor(1.0*len(set(G.neighbors(l)) & leads),1.0*len(set(G.neighbors(l)) & notleads)) for l in n_arm],1.0)
                            self.exploitation[n_arm] = (howmuch/len(newPotentials))*possible_arm_score + (1-(howmuch/len(newPotentials))) *P
                            if n_arm not in self.arm_hits:
                                self.arm_hits[n_arm] = [totalPulls+VIRTUAL, VIRTUAL]
                            else:
    ...
    def calculateExplorationValues(self, G, leads, v, notleads):
        ...
        for v in self.arm_hits:
            ...
            self.exploration[v] = self._getXplorationFactor(hits, max(total_pulls-self.arm_hits[v][0],hits))
...
class OPT_ARM(SN_UCB):
    ...
    def calculateExplorationValues(self, G, leads, v, notleads):
        if examine_arms:
            ...
            def calculateExploitationValues(self, G, leads, notleads, potentials):
                if examine_arms:
                    SN_UCB.calculateExploitationValues(self, G, leads, notleads, potentials)
    ...
    def getRanks(self):
        ...
        for v in leads:
            ...
            if maxRank == float('inf'):
                maxRank= max([r for r in ranks.values() if r != maxRank]+[0.0])
                for mi in maxItems:
    ...
    def updateTrail(self,v,isLead):
        ...
        if isLead:
            ...
            for l in self.leads_trail:
                self.leadTree[l]['v']=(self.leadTree[l]['v']*self.leadTree[l]['r'] + (0.0,1.0)[isLead]) / (self.leadTree[l]['r']+1)
                self.leadTree[l]['r']+=1
...
def updateInformationList(initialGraph,visibleG):
    """ updates the information of vertices in initialGraph """
    for v in initialGraph:
        updateInformation(v,visibleG)

def updateInformation(v,visibleG):
    """ updates the information by pulling profile v"""
    visibleG.add_node(v)
    newGroups = discover_neighbors(G, visibleG, v, CLOSED)
    if newGroups is None:
        return
    newProfiles = [p for g in newGroups for p in discover_neighbors(G, visibleG, g , CLOSED)]
...
if logInstancesEnabled:
    if not os.path.exists(instancesLogDir): os.makedirs(instancesLogDir)

if logGraphEnabled:
    if not os.path.exists(graphLogsDir): os.makedirs(graphLogsDir)
        ...
        for cycle in instancesSet:
            for instanceId in instancesSet[cycle]:
                ...
                if instanceId in leads:
                    instancesSet[cycle][instanceId]=instancesSet[cycle][instanceId]+['1']
                else:
                    instancesSet[cycle][instanceId]=instancesSet[cycle][instanceId]+['0']
...
#    Graph Logging
############################
graphLoggingHeader = ["#Iteration","Lcount","NLcount","PLcount","nodeCount","edgeCount","density","numberC","avgC","giantC","clusteringCoef","CC_target","Betweeness","Lratio","NLratio","PLratio"]
...
for section in sections:
    ...
    graph_folder = prompt_for_raw_option(ini,section,"graph_folder","Enter network folder path:")
    goal = prompt_for_option(ini,section,"goal","Goal ]:")
    net_gen = stored_graphs(lambda x:x.endswith(".csv") and (not x.startswith(resultfileprefix)))
    seed_counts = prompt_for_option(ini,section,"seed_counts","List the seed counts [int [,int [...]]]:")
    domain  = prompt_for_option(ini,section,"domain","List the Domain[tonic/comm]:")
    heuristics = prompt_for_option(ini,section,"heuristics","List the heuristics:")
    executions = prompt_for_option(ini,section,"executions","List of executions:")
    initials = prompt_for_option(ini,section,"initialGroups","initialGroups:")
    fieldnames = "source", "targetID", "n", "m", "density", "numberC", "avgC", "giantC", "clusteringCoef", "totalLeads","alg", "initialSeeds", "open", "closed", "profileID", "heuristic","lastIsLead", "IsLeadRequestsNum", "leadsFound", "notleadsFound", "time"
    resultf = csv.DictWriter(file(str(resultfileprefix) + str(gethostname())+"_%d.csv"%time.time(),"w"),fieldnames,"",lineterminator="\n")
    resultf.writeheader()
    stats = {}
    for seed_count in seed_counts:
        ...
        for G in net_gen:
            ...
            stats["avgC"] = sum(comps)/len(comps)
            stats["giantC"] = max(comps)
            stats["clusteringCoef"] = nx.algorithms.cluster.average_clustering(G)
            stats["totalLeads"]= 'NA'
            stats["lastIsLead"]= 'NA'
            random.seed(0)
            ...
            for OPEN in heuristics:
                ...
                for execution in executions:
                    ...
                    leadsFound = []
                    notLeads = []
                    initialProfiles = [gg for g in initialGroups for gg in G.neighbors(g)]
                    """Currently a single profile from each group"""
                    initialRepresentatives = [random.choice(G.neighbors(g)) for g in initialGroups]
                    visibleG = G.subgraph(initialProfiles + initialGroups)
                    CLOSED = set([])
                    OPEN = open_class(visibleG , initialRepresentatives)
                    updateInformationList(initialProfiles,visibleG)
                    instancesSet={}
                    graphSet = {}
                    if isLogEnabled():
                        if logInstancesEnabled & (reqestCount%loggingDeltaTime == 0): instancesSet[reqestCount] = createPotentialInstances(OPEN._open,visibleG,reqestCount) #log before running
                        if logGraphEnabled & (reqestCount%loggingDeltaTime == 0): graphSet[reqestCount] = createGraphInstance(visibleG,reqestCount,targetID)
                    for v,k in OPEN:
                        CLOSED.add(v)
                        updateInformation(v,visibleG) #TODO updates the information by pulling profile v
                        reqestCount= reqestCount+1
                        stats["profileID"] = v.strip()
                        stats["heuristic"] = k
                        stats["IsLeadRequestsNum"]= reqestCount
                        stats["leadsFound"] = len(leadsFound)
                    ...
                    if isLogEnabled():
                        if logInstancesEnabled & (reqestCount%loggingDeltaTime == 0): instancesSet[reqestCount] = createPotentialInstances(OPEN._open,visibleG,reqestCount) #log after network updates
                        if logGraphEnabled & (reqestCount%loggingDeltaTime == 0): graphSet[reqestCount] = createGraphInstance(visibleG,reqestCount,targetID)
                        if logInstancesEnabled: logInstances(targetID,OPEN,visibleG,instancesSet)
                        if logGraphEnabled: logGraph(targetID,OPEN,graphSet)
                    print "Done."
                    pass
